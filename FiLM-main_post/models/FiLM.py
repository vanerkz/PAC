...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.configs.ab == 2:
            ...
            self.projection = nn.Linear(configs.d_model*len(self.multiscale)*len(self.window_size), 1)
            self.distribution_presigma = nn.Linear((configs.d_model//2)*len(self.multiscale)*len(self.window_size), 100)
    ...
    def forward(self, x_enc,
        ...
        if self.configs.ab == 2:
            ...
            interalxout=torch.stack(interalx, dim=-1)
            out=torch.stack(x_decs, dim=-1)
            predmu = self.mlp(torch.stack(x_decs, dim=-1)).squeeze().permute(0,2,1)
            x,y,z,a=interalxout.shape
            interalxout=interalxout.reshape(x,y,-1)
            dec_out = self.distribution_presigma(interalxout)
            sigma = self.distribution_sigma(dec_out).permute(0,2,1)
